	...
	const createExtensionUIContext = (): ExtensionUIContext => ({
		select: (title, options, opts) =>
			createDialogPromise(
				opts,
				undefined,
				{ method: "select", title, options, timeout: opts?.timeout, metadata: opts?.metadata },
		...
		confirm: (title, message, opts) =>
			createDialogPromise(
				opts,
				false,
				{ method: "confirm", title, message, timeout: opts?.timeout, metadata: opts?.metadata },
		...
		input: (title, placeholder, opts) =>
			createDialogPromise(
				opts,
				undefined,
				{ method: "input", title, placeholder, timeout: opts?.timeout, metadata: opts?.metadata },
				(r) => ("cancelled" in r && r.cancelled ? undefined : "value" in r ? r.value : undefined),
			),
